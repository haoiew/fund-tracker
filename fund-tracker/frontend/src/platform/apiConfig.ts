...
export function resolveApiBaseUrl(): string {
  ...
  const isDesktopMode = shellMode === 'electron' || runtimeInfo.shell === 'electron'
  ...
  if (isDesktopMode) {
    return import.meta.env.VITE_DESKTOP_API_BASE_URL?.trim() || stored || configured || DEFAULT_NATIVE_API_BASE
  }
