...
const loadGrowthRates = async () => {
  ...
  try {
    ...
    // 计算3月增长率
    if (threeMonthData?.values && threeMonthData.values.length >= 2) {
    ...
    // 计算半年增长率
    if (sixMonthData?.values && sixMonthData.values.length >= 2) {
    ...
    // 计算年增长率
    if (oneYearData?.values && oneYearData.values.length >= 2) {
...
const loadHistoryData = async () => {
  ...
  try {
    ...
    console.log('正在获取历史数据:', fundData.value.code, timeRange.value)
    const response = await fundApi.getHistory(fundData.value.code, timeRange.value)
    ...
    // 确保数据长度一致
    const minLength = Math.min(data.dates.length, data.values.length)
    const dates = data.dates.slice(0, minLength)
    const values = data.values.slice(0, minLength).map((v: any) => {
      // 转换Decimal为number
      const num = typeof v === 'object' && v !== null ? parseFloat(String(v)) : Number(v)
      return isNaN(num) ? null : num
    }).filter((v: any) => v !== null)

    if (values.length === 0) {
      console.warn('没有有效的数值数据')
      loading.value = false
      return
    }

    console.log('处理后的数据:', { dates: dates.length, values: values.length })
    ...
    const option: echarts.EChartsOption = {
      tooltip: {
        ...
        formatter: (params: any) => {
          const p = params[0]
          return `${p.name}<br/>${t('home.fundList.nav')}: ${p.value?.toFixed(4) || '--'}`
        }
      },
      grid: {
        left: '3%',
...
const addToWatchlist = async () => {
  ...
  try {
    ...
    ElMessage.success(t('ocr.addSuccess', { name: fundData.value.name }))
  } catch {
...
onMounted(() => {
  if (visible.value && fundData.value) {
    nextTick(() => {
      initChart()
      loadHistoryData()
    })
